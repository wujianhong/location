...
fh = RotatingFileHandler("/usr/local/workspace/git/location/location1.log", mode='a', maxBytes=1024*1024*50, backupCount=10)
...
class Location():
    ...
    def get_user(self):
        ...
        with self.db.cursor() as cursor:
            sql = "select id, last_login_ip from user_back where id <= 1250000"
            cursor.execute(sql)
